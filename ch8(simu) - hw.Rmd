...
{
    a=runif(999,0,1)
    if(a[runif(1,0,length(a))]<=0.1)
        X[i]=1
    else if(0.1<a[runif(1,0,length(a))]&&a[runif(1,0,length(a))]<=0.4)
        X[i]=2
    else
        X[i]=5
}
...
p2=function(x)
{
    return(exp(-lamba)*lamba^x/gamma(x+1))
}
for(i in 1:999999)
    {
        x=runif(1,0,99)
...
```{r}
#when lamba=1,m=2,we can know:f(x)/h(x)/k=x*exp(-x/2)/8
X=c()
for(i in 1:999999)
{
    x=runif(1,0,100)
...
hist(X,breaks=200, freq=F, main="",xlim =range(0,10))
```
...
for(i in 1:9999)
{
    xi=runif(n)
    xbar=sum(xi)/n
    A[i]=sqrt(n)*(xbar-0.5)/sqrt(1/12)
}
hist(A,breaks=100, freq=F, main="")
```
...
for(i in 1:9999)
{
    ...
    Y=A[runif(1,1,length(A))]
    B[i]=max(X,Y)
}
hist(B,breaks=100, freq=F, main="")
```
...
car1/99999
car2/99999  #if the result is similar,there's no influence.
```
